...
class QuizCVCell: UICollectionViewCell {
    ...
    var question: Question? {
        didSet {
            ...
            if unwrappedQue.isAnswered {
                btnsArray[unwrappedQue.correctAns].backgroundColor=UIColor(red: 35/255, green: 183/255, blue: 159/255, alpha: 1.0)
                if unwrappedQue.wrongAns >= 0 {
                    btnsArray[unwrappedQue.wrongAns].backgroundColor=UIColor(red: 255/255, green: 99/255, blue: 71/255, alpha: 1.0)
                }
            }
        }
    }
    ...
    func setupViews() {
        ...
        addSubview(btn1)
        NSLayoutConstraint.activate([btn1.topAnchor.constraint(equalTo: lblQue.bottomAnchor, constant: 40), btn1.rightAnchor.constraint(equalTo: self.centerXAnchor, constant: -10), btn1.widthAnchor.constraint(equalToConstant: btnWidth), btn1.heightAnchor.constraint(equalToConstant: btnHeight)])
